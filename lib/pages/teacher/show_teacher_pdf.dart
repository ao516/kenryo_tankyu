...
class ShowTeacherPdfPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final List<Teacher> sortedTeacherList = ref.watch(teacherSortedListProvider);
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.done) {
                  if (snapshot.hasError) {
                    if(snapshot.error.toString().contains('object-not-found')) {
                      return const Center(child: Text('これから出てくる予定だよ！', style: TextStyle(fontSize: 20.0)));
                    } else {
      ...
      bottomNavigationBar: BottomAppBar(
        child: Row(
          ...
          children: [
            TextButton.icon(
                onPressed: () {
                  ref.read(selectedTeacherProvider.notifier).state = beforeTeacher;
                },
            ...
            TextButton.icon(
                ...
                onPressed: () {
                  ref.read(selectedTeacherProvider.notifier).state = afterTeacher;
                },
